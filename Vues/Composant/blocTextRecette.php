<div>
    <dt class="inline-flex items-center justify-center flex-shrink-0 w-12 h-12 mb-5 text-blue-600 rounded-full bg-blue-50">
    <img src="/assets/note.svg" alt="" class="h-10">
    </dt>
    <dd class="flex-grow">
        <h2 class="mb-3 text-lg font-medium tracking-tighter text-neutral-600">Note : </h2>
        <p class="text-base leading-relaxed text-gray-400"><?php echo $A_vue['note'];?> </p>
        
    </dd>
</div>
<div>
    <dt class="inline-flex items-center justify-center flex-shrink-0 w-12 h-12 mb-5 text-blue-600 rounded-full bg-blue-50">
    <img src="/assets/temps.svg" alt="" class="h-10">
    </dt>
    <dd class="flex-grow">
        <h2 class="mb-3 text-lg font-medium tracking-tighter text-neutral-600">Temps de Préparation : </h2>
        <p class="text-base leading-relaxed text-gray-400"><?php echo $A_vue['tempsPrepa'];?> </p>
        
    </dd>
</div>
<div>
    <dt class="inline-flex items-center justify-center flex-shrink-0 w-12 h-12 mb-5 text-blue-600 rounded-full bg-blue-50">
    <img src="/assets/cout.svg" alt="" class="h-10">
    </dt>
    <dd class="flex-grow">
        <h2 class="mb-3 text-lg font-medium tracking-tighter text-neutral-600">Coût : </h2>
        <p class="text-base leading-relaxed text-gray-400"><?php echo $A_vue['cout'];?> </p>
        
    </dd>
</div>
<div>
    <dt class="inline-flex items-center justify-center flex-shrink-0 w-12 h-12 mb-5 text-blue-600 rounded-full bg-blue-50">
    <img src="/assets/difficulte.svg" alt="" class="h-10">
    </dt>
    <dd class="flex-grow">
        <h2 class="mb-3 text-lg font-medium tracking-tighter text-neutral-600">Difficulté : </h2>
        <p class="text-base leading-relaxed text-gray-400"><?php echo $A_vue['difficulte'];?> </p>
        
    </dd>
</div>
<div>
    <dd class="flex-grow">
        <h2 class="mb-3 text-lg font-medium tracking-tighter text-neutral-600">Description : </h2>
        <p class="text-base leading-relaxed text-gray-400"><?php echo $A_vue['description'];?> </p>
        
    </dd>
</div>

<!--<dd class="flex-grow">
        <h2 class="mb-3 text-lg font-medium tracking-tighter text-neutral-600">Note : </h2>
        <p class="text-base leading-relaxed text-gray-400"> </p>
        <a href="#" class="inline-flex items-center mt-6 font-semibold text-blue-500 md:mb-2 lg:mb-0 hover:text-neutral-600" title="read more">
            Lorem ipsum.
            <svg class="w-4 h-4 ml-2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="20" height="20" fill="currentColor">
                <path fill="none" d="M0 0h24v24H0z"></path>
                <path d="M16.172 11l-5.364-5.364 1.414-1.414L20 12l-7.778 7.778-1.414-1.414L16.172 13H4v-2z"></path>
            </svg>
        </a>
    </dd> -->